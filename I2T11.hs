...
parsePRIVMSG :: T.Text -> Maybe T.Text
parsePRIVMSG x = (getElem 2 . T.words $ x) -- channel
...
parseMsg :: T.Text -> T.Text -> Maybe T.Text
parseMsg nick x = 
    if | T.isInfixOf " PRIVMSG #" x -> if | T.isInfixOf nick x -> Just "\128994" <> (parsePRIVMSG x)
                                          | otherwise -> parsePRIVMSG x
       | T.isInfixOf " PRIVMSG " x -> Just "\128994" <>  ((T.unwords . L.tail . T.words) <$> (parsePRIVMSG x))
       | T.isPrefixOf "PING" x -> Nothing
       | T.isInfixOf " QUIT " x -> Nothing
       | T.isInfixOf " JOIN " x -> Nothing
       | T.isInfixOf " PART " x -> Nothing
       | T.isInfixOf ":*.net*.split" x -> Nothing
       | (T.isInfixOf " NICK " x) && (T.isInfixOf nick x) -> Just "newNick" <> (getElem 2 . T.words $ x) -- Just "newNick:nick"
       | T.isInfixOf nick x -> Just $ "\128994" <> x
       | otherwise -> Just x

relayIRC2Tele :: Token -> Manager -> ChatId -> Socket -> ThreadId -> T.Text -> IO a
